...
const start = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const studio = session.studios.find(
      (e) => e.ownerId === session.settings.draft.order[0],
    );
...
const auto = publicProcedure
  ...
  .input(
    z.object({ sessionId: z.string(), studioId: z.string(), pick: z.number() }),
  )
  .output(z.void())
  .mutation(async ({ ctx, input }) => {
...
async function makePick(ctx: TRPCContext, input: z.infer<typeof makePickObj>) {
  ...
  await draftEvent<typeof draftState>(
    `draft:${input.sessionId}:draft-update`,
    draftState,
  );
  return input.sessionId;
...
export async function autoDraft(
  ...
  if (
    picks.length ===
    session?.settings.draft.numRounds * session?.studios.length
  )
    return;
  ...
  const bestAvailable = films?.results.filter(
    (e) => !unavailableIds.includes(e.id),
  )[0];
